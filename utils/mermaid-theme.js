...
import { getMermaidDiagramKeyword } from './content-detection.js';

const FONT_FAMILY = 'Inter, "PingFang SC", "Microsoft YaHei", "Noto Sans CJK SC", sans-serif';
const DARK_TEXT = '#1D1D18';
...
export const getMermaidThemePalette = (theme) => {
  ...
  return {
    ...
    mutedForeground: '#424238',
    surface: '#FFFFFF',
    surfaceMuted: '#EEEDE6',
    nodeFill: '#F5F5F0',
    secondaryFill: '#EEEDE6',
    tertiaryFill: '#FFFFFF',
...
const buildThemeVariables = (theme) => {
  ...
  return {
    ...
    nodeBorder: palette.foreground,
    nodeTextColor: palette.foreground,
    arrowheadColor: palette.foreground,
    border2: palette.foreground,
    clusterBkg: palette.surfaceMuted,
    clusterBorder: palette.foreground,
...
const hasClassNameMatching = (element, matcher) => {
  if (!(element instanceof Element)) return false;
  return (element.getAttribute('class') || '').split(/\s+/).some((className) => matcher.test(className));
};
...
const hasSelfOrAncestorClassNameMatching = (element, matcher) => {
  let probe = element instanceof Element ? element : null;
  while (probe) {
    if (probe.matches?.('svg')) break;
    if (hasClassNameMatching(probe, matcher)) return true;
    probe = probe.parentElement;
  }
  return false;
};

const isBlockDiagramClassName = /block/i;
const isNonBlockDiagramClassName = /packet/i;
const isBlockDiagramElement = (element) =>
  hasSelfOrAncestorClassNameMatching(element, isBlockDiagramClassName) &&
  !hasSelfOrAncestorClassNameMatching(element, isNonBlockDiagramClassName);

const isBlockArrowElement = (element) =>
  hasSelfOrAncestorClassNameMatching(element, /arrow|edge|link|line/i);

const isBlockDiagramSvg = (svgElement) =>
  svgElement?.getAttribute?.('aria-roledescription') === 'block' ||
  Boolean(svgElement?.querySelector?.('g.block'));
...
const normalizeLineAndBorderColors = (svgElement, palette) => {
  for (const element of svgElement.querySelectorAll(LINEWORK_SELECTOR)) {
    if (isSankeyLinkElement(element)) continue;
...
const isSankeyLinkElement = (element) =>
  Boolean(element.closest?.('g.links g.link'));
...
const normalizeBlockDiagramColors = (svgElement, palette) => {
  if (!isBlockDiagramSvg(svgElement)) return;

  const nodeFill = palette.nodeFill;
  const nodeStroke = palette.foreground;
  const arrowColor = palette.foreground;
  const labelColor = palette.foreground;
  const handledShapes = new WeakSet();

  for (const node of svgElement.querySelectorAll('g.node,g.block,g[class*="block"]')) {
    if (hasSelfOrAncestorClassNameMatching(node, /packet/i)) continue;
    if (hasSelfOrAncestorClassNameMatching(node, /arrow|edge|link|line/i)) continue;

    for (const shape of node.querySelectorAll('rect,polygon')) {
      if (shape.closest('defs,marker')) continue;
      if (isBlockArrowElement(shape)) continue;

      handledShapes.add(shape);
      setElementFillColor(shape, nodeFill);
      setElementStrokeColor(shape, nodeStroke);
    }

    for (const label of node.querySelectorAll('text,tspan,foreignObject,foreignObject *')) {
      if (!label.textContent?.trim()) continue;

      setElementTextColor(label, labelColor);
    }
  }

  for (const shape of svgElement.querySelectorAll('rect,polygon')) {
    if (handledShapes.has(shape)) continue;
    if (!isBlockDiagramElement(shape) || shape.closest('defs,marker')) continue;
    if (isBlockArrowElement(shape)) continue;
  ...
  for (const line of svgElement.querySelectorAll('path,polygon,line,polyline')) {
    if (!isBlockDiagramElement(line) || line.closest('defs,marker')) continue;
    if (!isBlockArrowElement(line)) continue;

    setElementStrokeColor(line, arrowColor);
    const fill = getElementColorValue(line, 'fill');
    if (fill && fill.toLowerCase() !== 'none') {
      setElementFillColor(line, arrowColor);
    }
  }

  for (const markerPart of svgElement.querySelectorAll('marker path,marker polygon')) {
    setElementFillColor(markerPart, arrowColor);
    setElementStrokeColor(markerPart, arrowColor);
  }

  for (const label of svgElement.querySelectorAll('text,tspan,foreignObject,foreignObject *')) {
    if (!isBlockDiagramElement(label)) continue;
    if (!label.textContent?.trim()) continue;

    setElementTextColor(label, getReadableTextColor(findNearestShapeFill(label) || nodeFill, palette));
  }
};
...
const normalizeDarkSankeyColors = (svgElement, palette) => {
  if (palette.theme !== 'dark') return;
  if (!svgElement.querySelector('.sankey-label-bg,.sankey-label-fg,g.links g.link')) return;

  for (const link of svgElement.querySelectorAll('g.links,g.links g.link,g.links g.link path')) {
    link.style.mixBlendMode = 'normal';
    setInlineStyleValue(link, 'mix-blend-mode', 'normal');
    link.setAttribute('stroke-opacity', '0.72');
    link.style.strokeOpacity = '0.72';
  }

  for (const backgroundLabel of svgElement.querySelectorAll('text.sankey-label-bg')) {
    setElementFillColor(backgroundLabel, 'none');
    setElementStrokeColor(backgroundLabel, palette.background);
    setElementStrokeWidth(backgroundLabel, '4');
    backgroundLabel.setAttribute('stroke-linejoin', 'round');
    backgroundLabel.setAttribute('paint-order', 'stroke');
    backgroundLabel.style.strokeLinejoin = 'round';
    backgroundLabel.style.paintOrder = 'stroke';
  }

  for (const foregroundLabel of svgElement.querySelectorAll('text.sankey-label-fg')) {
    setElementTextColor(foregroundLabel, palette.foreground);
    setElementStrokeColor(foregroundLabel, 'none');
  }

  for (const label of svgElement.querySelectorAll('g.node-labels text:not(.sankey-label-bg):not(.sankey-label-fg)')) {
    setElementTextColor(label, palette.foreground);
    setElementStrokeColor(label, 'none');
  }
};
...
const normalizeDarkSpecialDiagramColors = (svgElement, palette) => {
  ...
  normalizeDarkSankeyColors(svgElement, palette);
};
...
const normalizeSvgContrast = (svgElement, theme) => {
  const palette = getMermaidThemePalette(theme);
  normalizeBlockDiagramColors(svgElement, palette);
...
const markupAttrsBelongToBlockDiagram = (attrs) =>
  /class="[^"]*block[^"]*"/i.test(attrs) && !/class="[^"]*packet[^"]*"/i.test(attrs);

const forceMarkupBlockDiagramColors = (markup, palette) => {
  const nodeFill = palette.nodeFill;
  const nodeStroke = palette.foreground;
  const arrowColor = palette.foreground;
  const labelColor = palette.foreground;
  ...
  let adjusted = markup.replace(
    /<(rect|polygon)\b([^>]*class="[^"]*block[^"]*"[^>]*)(\/?)>/gi,
    (match, tagName, attrs, selfClose) => {
      if (!markupAttrsBelongToBlockDiagram(attrs) || /arrow|edge|link|line/i.test(attrs)) return match;
      return `<${tagName}${upsertMarkupStyle(attrs.replace(/\s(?:fill|stroke)="[^"]*"/gi, ''), {
        fill: nodeFill,
        stroke: nodeStroke,
      })} fill="${nodeFill}" stroke="${nodeStroke}"${selfClose ? ' /' : ''}>`;
    },
  );

  adjusted = adjusted.replace(
    /<(path|polygon|line|polyline)\b([^>]*class="[^"]*block[^"]*"[^>]*)(\/?)>/gi,
    (match, tagName, attrs, selfClose) => {
      if (!markupAttrsBelongToBlockDiagram(attrs)) return match;
      if (!/arrow|edge|link|line/i.test(attrs)) return match;
      return `<${tagName}${upsertMarkupStyle(attrs.replace(/\s(?:fill|stroke)="[^"]*"/gi, ''), {
        stroke: arrowColor,
        fill: arrowColor,
      })} stroke="${arrowColor}" fill="${arrowColor}"${selfClose ? ' /' : ''}>`;
    },
  );

  adjusted = adjusted.replace(
    /<(text|tspan)\b([^>]*class="[^"]*block[^"]*"[^>]*)>/gi,
    (match, tagName, attrs) => {
      if (!markupAttrsBelongToBlockDiagram(attrs)) return match;
      return `<${tagName}${upsertMarkupStyle(attrs.replace(/\sfill="[^"]*"/gi, ''), {
        fill: labelColor,
        color: labelColor,
      })} fill="${labelColor}">`;
    },
...
const forceDarkMarkupSpecialColors = (markup, palette) => {
  ...
  adjusted = adjusted.replace(
    /<g\b([^>]*class="[^"]*\blink\b[^"]*"[^>]*)>/gi,
    (_match, attrs) => `<g${upsertMarkupStyle(attrs, {
  ...
  adjusted = adjusted.replace(
    /<text\b([^>]*class="[^"]*\bsankey-label-bg\b[^"]*"[^>]*)>/gi,
    (_match, attrs) =>
      `<text${upsertMarkupStyle(attrs.replace(/\s(?:fill|stroke|stroke-width|stroke-linejoin|paint-order)="[^"]*"/gi, ''), {
        fill: 'none',
        stroke: palette.background,
        'stroke-width': '4',
        'stroke-linejoin': 'round',
        'paint-order': 'stroke',
      })} fill="none" stroke="${palette.background}" stroke-width="4" stroke-linejoin="round" paint-order="stroke">`,
  );

  adjusted = adjusted.replace(
    /<text\b([^>]*class="[^"]*\bsankey-label-fg\b[^"]*"[^>]*)>/gi,
    (_match, attrs) =>
      `<text${upsertMarkupStyle(attrs.replace(/\s(?:fill|stroke)="[^"]*"/gi, ''), {
        fill: palette.foreground,
        color: palette.foreground,
        stroke: 'none',
      })} fill="${palette.foreground}" stroke="none">`,
  );
...
const applyStringBasedContrast = (svg, theme = 'light') => {
  ...
  adjusted = adjusted.replace(/\sstroke="(?!none|transparent|url\()[^"]*"/gi, ` stroke="${palette.foreground}"`);
  adjusted = adjusted.replace(/stroke\s*:\s*(?!none|transparent|url\()[^;"]+/gi, `stroke:${palette.foreground}`);
  adjusted = adjusted.replace(/(<marker\b[^>]*>)([\s\S]*?)(<\/marker>)/gi, (_match, open, content, close) => {
    const markerContent = content
      ...
      .replace(/\sstroke="(?!none|transparent|url\()[^"]*"/gi, ` stroke="${palette.foreground}"`)
      .replace(/stroke\s*:\s*(?!none|transparent|url\()[^;"]+/gi, `stroke:${palette.foreground}`);
    return `${open}${markerContent}${close}`;
  });

  adjusted = forceMarkupTextColor(adjusted, palette.foreground);
  adjusted = forceMarkupBlockDiagramColors(adjusted, palette);
...
export const getMermaidDiagramScale = (code) => {
  const diagramKeyword = getMermaidDiagramKeyword(code);

  if (diagramKeyword === 'gantt' || diagramKeyword === 'packet-beta') return 1.1;
  if (
    diagramKeyword === 'graph' ||
    diagramKeyword === 'flowchart' ||
    diagramKeyword === 'ishikawa-beta'
  ) {
    return diagramKeyword === 'ishikawa-beta' ? 0.72 : 0.576;
  }
  if (diagramKeyword === 'requirementdiagram') return 0.8;
  return MERMAID_DIAGRAM_SCALES[diagramKeyword] ?? 1;
};
