...
const createRenderTimeoutError = (task) => {
  const error = new Error(`Mermaid render timed out after ${activeRenderTimeoutMs}ms.`);
  error.name = 'MermaidRenderTimeoutError';
  error.code = 'MERMAID_RENDER_TIMEOUT';
  error.theme = task.theme;
  return error;
};
...
async function runRenderTask(task) {
  ...
  const result = await withRenderTimeout(mermaid.render(id, task.code), task);
