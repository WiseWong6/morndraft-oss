...
|---|---:|---|---|---|---|---|---|---|
| caniuse-lite | 1.0.30001810 | CC-BY-4.0 | transitive via Browserslist/Babel/Vite | No; dev/build target data only | No | Attribution required if redistributed; keep package notice in third-party register | Approved | node_modules/caniuse-lite/LICENSE; npm explain caniuse-lite |
| dompurify | 3.4.13 | (MPL-2.0 OR Apache-2.0) | transitive via mermaid | Yes; runtime dependency tree for Mermaid rendering | No | Use Apache-2.0 license path; keep notice/attribution; no source availability obligation for MornDraft code under Apache path | Approved | node_modules/dompurify/LICENSE; npm explain dompurify |
...
| minimatch | 10.2.6 | BlueOak-1.0.0 | transitive via @eslint/config-array / eslint | No; dev/lint tooling only | No | Keep license link or notice in third-party register; no source availability obligation for MornDraft code | Approved | node_modules/minimatch/package.json; npm explain minimatch |
